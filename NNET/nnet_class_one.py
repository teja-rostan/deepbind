...
def learn_and_score(scores_file, delimiter, target_size):
    ...
    data, target, _, target_class = get_data_target.get_original_data(scores_file, delimiter, target_size, "class")
    ...
    max_len = get_max_len(target, target_class, target_size)
    ...
    for t in np.hstack([range(wild_type), range(wild_type+1, target_size)]):
        ...
        # targets, ids_b, data_b = get_balanced_data(t, class_size, target_c, data, target, max_len, ids)  # balanced data
        targets, data_b, ids_b = target[:, class_size * t:class_size * t + class_size], data, ids  # unbalanced data
